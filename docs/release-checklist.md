...
- This verifies Stretchly settings-key and command coverage, `git diff --check`, `swift test`, app build, `shouldrest://` URL scheme, bundle identifier, localization and sound resources, strict app codesign, first-run GUI smoke against a temporary support directory, ad-hoc DMG creation, and `hdiutil verify`.
